...
class Map {
    ...
    void set(int x, int y, const char toSet) {
		if (toSet != HERO or toSet != MONSTER or toSet != WALL or toSet != WATER or toSet != OPEN or toSet != TREASURE) return;
		map.at(x).at(y) = toSet;
    }

	void save(const Map, const string& filename) {
		ofstream saves(filename);
		if (!saves) cout << "Failed to save File" << endl;
		for (auto& row : map) {
	...
	void load( const string& filename) {
		...
		if (!load) cout << "Error loading file" << endl;
		for (auto &row : map) {
			for (auto &col : row) {
				col = load.get();
				if(col == '\n') {
